...
static void CheckAlignmentSetting(GtkWidget *widget)
{
	...
	if( !strcmp(GetModelName(), "IP4200") || !strcmp(GetModelName(), "MP500") ||
	    ...
	    !strcmp(GetModelName(), IDM_IP_3300 ) || 
	...
	else if ( !strcmp(GetModelName(), IDM_MP_510 ) ) {
...
{
	{"PIXMAIP1000", create_PX550Iregi_dialog,   NULL, NULL                      },		// 2006/05/20
	{"PIXMAIP1500", create_PX850Iregi1_dialog,  NULL, NULL                      },
	{"PIXUSIP3100", create_PX850Iregi1_dialog,  create_PX850Iregi2_dialog, NULL },
	{"PIXUSIP4100", create_PX860Iregi1_dialog,  create_PX860Iregi2_dialog, NULL },
	{"PIXUSIP8600", create_ip8600regi_dialog,   NULL, NULL                      },
	{"IP2200", 		create_IP2200regi_dialog,   NULL, NULL                      },
	{"IP4200", 		create_IP4200regi1_dialog,  create_IP4200regi2_dialog, NULL },
	{"IP6600D", 	create_IP6600Dregi1_dialog, create_IP6600Dregi2_dialog, NULL },
	{"IP7500", 		create_IP7500regi1_dialog,  create_IP7500regi2_dialog, NULL },
	{"MP500", 		create_IP4200regi1_dialog,  create_IP4200regi2_dialog, NULL },
	{IDM_MP_160,	create_IP2200regi_dialog,   NULL, NULL                      }, // 2006/06/27
	{IDM_IP_3300,	create_IP3300regi1_dialog,  create_IP3300regi2_dialog, NULL }, // 2006/05/29
	{IDM_MP_510,	create_IP3300regi1_dialog,  create_IP3300regi2_dialog, NULL }, // 2006/05/29
	{IDM_IP_4300,	create_IP4300regi1_dialog,  create_IP4300regi2_dialog, create_IP4300regi3_dialog }, // 2006/05/26
	{IDM_MP_600,	create_IP4300regi1_dialog,  create_IP4300regi2_dialog, create_IP4300regi3_dialog }, // 2006/05/26
	{IDM_IP_90,	create_IP90regi_dialog,  NULL, NULL }, // 2006/06/29
	{IDM_IP_2500,	create_IP2200regi_dialog,   NULL, NULL                      }, // 2006/07/06
	{IDM_IP_1800,	create_IP2200regi_dialog,   NULL, NULL                      }, // 2006/07/06
	{NULL, NULL}
};
									 ...
									 gpointer user_data)
{
	...
	if (GTK_TOGGLE_BUTTON(togglebutton)->active)
	  current_drylevel_value = getnum(user_data);
	...
	if( !strcmp(model_name,"BJS600") || !strcmp(model_name,"BJS630")
		...
		gtk_main();									
  		...
  		if (result != ID_OK)		
			...
			!strcmp(model_name, IDM_IP_3300) || 			// 2006/05/16
	...
	if (!strcmp(model_name, "BJF850") | !strcmp(model_name, "BJF860") |
		...
		*p = 0x00;
		PutPrintData(buf, n+BJLLEN+BJLSTARTLEN+
			 ...
			 !strcmp(model_name,"IP6600D")|| // 2006/05/16 
			 !strcmp(model_name,IDM_MP_160) ||
		...
		*p = 0x00;
		...
		if (result != ID_CANCEL)
			UtilCleaning(uidb);
			 ...
			 !strcmp(model_name, IDM_IP_2500) || !strcmp(model_name, IDM_IP_1800) ){	// 2006/05/30
...
void UtilHeadAdjust(LPUIDB uidb)
{
		...
		if( !manual_head_mode ){
			...
			*p = 0x00;
			...
			for (i=0; i<nKindCnt; i++) {	
				...
				nValue = ItemValue[i];		
				sprintf(temp, BJLSETBANDREG, lpBJLtbl[i], nValue<0? '-':(nValue>0? '+':'0'), abs(nValue));
				memcpy(p, temp, BJLSETBANDREGLEN); p += BJLSETBANDREGLEN;
			}
			memcpy(p, BJLEND, BJLENDLEN); p+=BJLENDLEN;	
			*p = 0x00;			
	...
	else if( !strcmp(model_name,IDM_IP_2500) || !strcmp(model_name,IDM_IP_1800) )	// 2006/07/06
	{
		int 	n;
		int fname_length;
		char fnamebuf[256];
	...
	else if( !strcmp(model_name,IDM_MP_160) )	// 2006/05/30
	{
		GtkWidget *head_alignment_dialog;
		GtkWidget *execute_button;
		int 	n;
		...
		gtk_label_set_text(
			...
			LookupText(g_keytext_list, "utility_message_65"));
	...
	else if( !strcmp(model_name,"IP6600D") )
	{
		...
		int 	n;
			...
			if ( !strcmp(model_name, IDM_IP_3300) ){
				...
				gtk_label_set_text(
					GTK_LABEL(LookupWidget(head_alignment_dialog, "head_alignment_label1")),
					LookupText(g_keytext_list, "utility_message_62"));
			}
			else {
				gtk_label_set_text(
					GTK_LABEL(LookupWidget(head_alignment_dialog, "head_alignment_label1")),
	...
	else if( !strcmp(model_name, IDM_IP_4300) || !strcmp(model_name, IDM_MP_600) ) // 2006/05/30
	{
		...
		if( !manual_head_mode ){
			...
			*p = 0x00;
			...
			gtk_label_set_text(
				GTK_LABEL(LookupWidget(head_alignment_dialog, 
									   "head_alignment_label1")),
				LookupText(g_keytext_list, "utility_message_59")); 	// 2006/05/25
			gtk_widget_show(head_alignment_dialog);
			gtk_window_set_transient_for(GTK_WINDOW(head_alignment_dialog),
								 GTK_WINDOW(UI_DIALOG(g_main_window)->window));
			execute_button = LookupWidget(head_alignment_dialog, 
			...
			*p = 0x00;			
			PutPrintData(buf,
						 BJLLEN+BJLSTARTLEN+BJLCTRLMODELEN
						 +(BJLSETREG2LEN*nKindCnt)+BJLENDLEN+1);
		...
		gtk_label_set_text(
			GTK_LABEL(LookupWidget(head_alignment_dialog, "head_alignment_label1")),
			LookupText(g_keytext_list, "utility_message_68"));
...
void UtilSetConfig(LPUIDB uidb)
{
	...
	if (!strcmp(model_name, "BJF900") || !strcmp(model_name, "BJF9000") ||
		...
		!strcmp(model_name, "IP7500") ||
		!strcmp(model_name, "IP6600D")||
		!strcmp(model_name, IDM_MP_160 ) ||	// 2006/05/30
...
//		fprintf( stderr, "poweroffac_index : %d ||| poweroffbattery_index : %d\n", poweroffac_index ,poweroffbattery_index );
		poweroff_combo_ac = LookupWidget(autopower_dialog, "autopower_type2_combo1");	
		SetComboBox(poweroff_combo_ac, AutoPowerOffKey, 6, poweroffac_index);		
		poweroff_combo_battery = LookupWidget(autopower_dialog, "autopower_type2_combo2");	
		SetComboBox(poweroff_combo_battery, AutoPowerOffBatteryKey, 4, poweroffbattery_index);
...
//		fprintf( stderr, "poweron_index : %d ||| poweroff_index : %d\n", poweron_index ,poweroff_index );
...
void UtilPlateCleaning(LPUIDB uidb)
{
	...
	else if ( !strcmp(model_name, IDM_MP_160) ||		// 2006/06/20
...
void SetComboBox(GtkWidget *combo, char **Keytbl, int n, int index)
{
	...
	/* Ver.2.70 */
	current = g_list_nth_data(popdown, index);
...
//	gtk_combo_set_popdown_strings(GTK_COMBO(combo), popdown);	
	SetPopdownStringsToCombo(combo, popdown, index_p);	/* Ver.2.70 */
	/* Ver.2.70 end */
