...
PRIVATE guint		gCartridgeWidth[CARTRIGE_TYPE_NUM] = {108, 57, 0, 74, 82, 40, 40, 108, 99, 125, 142, 41, 41, 41, 41, 103, 114, 141, 84, 89, 41, 41, 52, 52  };	
                                                         // ip2200(black), ip2200(color), ip2200(black_hc), ip2200(color_hc), ip4200, ip6600D, ip7500, ip3300, ipxxxx, ip2200(black_mn), ip2200(color_mn), ip90, ip90(CIL:OFF)
...
//	 {-1, 0}},
	...
	{{ID_PIXMAP_BB, 27},		// for ip3300 type.
	 ...
	 {-1, 0},
	 {-1, 0}},
	{{ID_PIXMAP_C2,  48},		// for ipxxxx type.
	 {ID_PIXMAP_B,  41},
	 {-1, 0},
	 {-1, 0},
	 {-1, 0},
...
PRIVATE gchar *gCartridgeXPMFileSet[CARTRIGE_CLASS_NUM][CARTRIDGE_XPM_SET_NUM] = {
	...
	{STR_PIXMAP_FILE_NAME_BLACK_BB_26,			// for ip3300 type (exchange:no).
	 ...
	 NULL,
	 NULL},
	{STR_PIXMAP_FILE_NAME_BLACK_B_26,		// for ipxxxx type
...
/*** Parameters end ***/
	...
	//for Ver.2.70 iP90 black
		...
		if (inkLevel == ID_INK_LEVEL_70) {
			...
			//Ver.2.70 for ????????????
			|| pCartridgeClass->type == ID_CARTRIDGE_TYPE_COLOR_ipxxxx
			|| pCartridgeClass->type == ID_CARTRIDGE_TYPE_COLOR_ip2200
			|| pCartridgeClass->type == ID_CARTRIDGE_TYPE_BLACK_MN_ip2200
			|| pCartridgeClass->type == ID_CARTRIDGE_TYPE_COLOR_MN_ip2200
			|| pCartridgeClass->type == ID_CARTRIDGE_TYPE_BLACK_HC_ip2200
...
PRIVATE void drawIconPixmap(GtkWidget *widget, ST_CartridgeClass *pCartridgeClass, ENUM_CartridgePixmapID colorID, ENUM_InkStatusID stat, ENUM_STSMessageID *msgid, gint msgnum, guint xoffset, guint yoffset )
{
	...
	for (i = 0; i < msgnum; i++) {
		if( (*(msgid + i) == ID_STS_MESSAGE_OC_PLURALERROR_TANK )
		 || (*(msgid + i) == ID_STS_MESSAGE_OC_DIFFERENT_REGION_TANK )
		 || (*(msgid + i) == ID_STS_MESSAGE_OC_POSITION_MISMACH_TANK )
		 || (*(msgid + i) == ID_STS_MESSAGE_OC_UNINSTALLATION_TANK )
		 || (*(msgid + i) == ID_STS_MESSAGE_OC_HARDWAREERROR_TANK ) ) {
			noticondraw_flg = 1;
			break;
		}
	}
	...
	switch( stat ) {
		...
		case ID_INK_STS_UNKNOWN: //for iP90
			...
			break;
		//Ver.2.70
		case ID_INK_STS_IUK: //for iPxxxx only
			if( colorID == ID_PIXMAP_BB ) {
				pix = &gpIconOutW;
				bmp = &gpMaskIconOutW;
			}
			...
			else if( colorID == ID_PIXMAP_C2 ){ //for iPxxxx
				pix = &gpIconOutD2;
				bmp = &gpMaskIconOutD2;
			}
			else {
