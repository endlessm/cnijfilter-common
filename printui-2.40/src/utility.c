...
static void SetDryLevel(void)
{
	...
	char		*p=buf;
	char 		temp[32];
	int 		total_bytes, temp_bytes, cmd_bytes;
	char		*model_name;
	...
	if( config_flag & SETCONFIG_ABRASION ){
		temp_bytes = sprintf(temp, BJLPAPERGAP, PaperGapString[papergap_mode]);
...
void UtilCleaning(LPUIDB uidb)
{
	...
	if( !strcmp(model_name,"BJS600") || !strcmp(model_name,"BJS630")
		...
		if( InkMsgInfo[index].GroupFlag )
			gtk_label_set_text(
					GTK_LABEL(LookupWidget(cleaning_dialog, "cleaning_label")),
	...
	if (UtilMessageBox(LookupText(g_keytext_list, "utility_message_7"),
			...
			!strcmp(model_name,"PIXUS990I") )
		{
			...
			*p = 0x00;
			...
			strcpy( fnamebuf, BJTESTFILEPATH );
			for( i=0; i<=strlen(model_name); i++)
					bufModelName[i] = tolower( model_name[i] );	
			strcat( fnamebuf, bufModelName );
			strcat( fnamebuf, "/");
			strcat( fnamebuf, GetPatternFileName(NOZZLEPATTERN) );
	...
	if (!strcmp(model_name,"BJF850") || !strcmp(model_name,"BJF860")
	 || !strcmp(model_name,"BJF870")) {
		...
		*p = 0x00;			
		...
		msg = LookupText(g_keytext_list, "utility_message_29");
		...
		*p = 0x00;
		...
		strcat( fnamebuf, bufModelName );
		strcat( fnamebuf, "/");
		strcat( fnamebuf, GetPatternFileName(REGIPATTERN1) );
		fname_length = strlen(fnamebuf) + 1;
		PutFileData( buf, n+1, fnamebuf, fname_length );
...
void UtilInkReset(LPUIDB uidb)
{
	...
	if (strcmp(model_name,"BJS300") && strcmp(model_name,"I250")
		&& strcmp(model_name,"I255") )
		return;
							...
							short *ItemValue, short page )
{
	...
	/* Bscc File Name */
	strcpy( bscc_file, BSCCFILEPATH );
	strcat( bscc_file, BsccRegInfo[index].bscc_file ); 

	pRegTbl = &BsccRegInfo->tBsccRegTbl[page];

	/* Create BSCC REG Command */
	strcpy(bscc_buf, BSCCREGTAG );
