...
PRIVATE const gchar	*gSTSMessageTable[] = {
	...
	N_("Document printing cannot be executed because the paper output tray is closed.\n"),
	/* ID_STS_MESSAGE_BUSY_HEADPROTECT_INKCOVER */
	N_("The ink cannot be replaced because the print head is in a standby condition.\nClose the printer's ink cover, wait for a while, and then replace the ink.\n"),
	/* ID_STS_MESSAGE_CARTRIDGE_NONE_INKCOVER */
	N_("A print head is not installed or the installed print head is not compatible with this printer.\nInstall an appropriate print head and close the printer's ink cover.\n"),
	/* ID_STS_MESSAGE_OC_COVER_INKCOVER */
	N_("The printer's ink cover is open.\nIf printing is in progress, the ink tank cannot be replaced. Close the ink cover, wait for printing to finish, then open the ink cover again and replace the ink tank.\n\n"),
	/* ID_STS_MESSAGE_OC_USDEVICE_DIRECTPRINT5 */
	N_("An unsupported device is connected to the USB flash drive port. Disconnect the device from the USB flash drive port. If the device is connected via a USB hub, unplug the USB hub, and then connect the device directly to the USB flash drive port.\n"),
	/* ID_STS_MESSAGE_OC_HARDWAREERROR_INKCOVER */
	N_("An ink tank cannot be recognized.\nOpen the printer's ink cover and replace the ink tank on which the ink lamp is off.\n"),
	/* ID_STS_MESSAGE_SC_LFSERVICEERR_WM */
	N_("A printer error has occurred.\nTurn the printer off. Then turn it on after opening the paper output tray.\nIf this doesn't clear the error, the printer needs repair. See the manual and contact the service center.\n"),
	/* ID_STS_MESSAGE_OC_PAPER_JAM1_2nd_2 */
	N_("Remove the jammed paper slowly with both hands.\n"),
	/* ID_STS_MESSAGE_OC_PAPER_JAM_BACK12_3 */
	N_("Remove the jammed paper from inside the transport unit slowly with both hands.\nIf the paper cannot be removed, refer to the manual to remove it.\n"),
	/* ID_STS_MESSAGE_OC_PAPER_JAM1_2_2_2nd */
	N_("Remove the jammed paper slowly with both hands.\n"),
	/* ID_STS_MESSAGE_ESTIMATEDINKLEVEL_LABEL */
	N_("Estimated ink levels"),
};
...
PRIVATE const gchar	*gOtherMessageTable[] = {
	...
	"  cngpijmon(3.10.01.013) : argument error.\n  Usage: cngpijmon****** [printer name]\n\n   ex) cngpijmonmx920 MX920\n\n",
};
...
/*** Parameters end ***/
		...
		if (pPrinterStatus != NULL) {
			...
			||  pPrinterStatus->cartridgeClass[1].type >= ID_CARTRIDGE_TYPE_PHOTO) {
				...
				if (i == CARTRIDGE_COLOR_TYPE_NUM) {
					...
					/* for Estimated Ink Level */
					if( pictDrawFlg == 1 ){
						memset(tempBuf, 0, sizeof(tempBuf));
						strncpy(tempBuf, dgettext(PACKAGE, N_("Estimated ink levels")), MAX_BUF_SIZE-1);
						setLabelString(STR_MAIN_LABEL_INKLEVELSTR, tempBuf);
					}
				}
			}
			else {
	...
	/* Ver.3.90 Delete Estimated Ink Level Label */
	if( pictDrawFlg == FALSE ){
		/* for Estimated Ink Level */
		memset(tempBuf, 0, sizeof(tempBuf));
		strncpy(tempBuf, "                                                ", MAX_BUF_SIZE-1);
