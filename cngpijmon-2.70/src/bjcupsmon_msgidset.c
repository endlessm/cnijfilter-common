...
static int setFrontFeederID(ST_CUPSSTS *p_tbl);
...
ST_MSGTBL busystat[]={
  ...
  {'N', ID_STS_MESSAGE_BUSY_TANK_CHANGE},
  {ENDTAG, 0}
};
...
//Ver.2.70
static const
ST_FRONTFEEDER_TBL frontfeeder_tbl[]={
  {"IP2200",  NO_FRONT_FEEDER},
  {"IP4200",  NO_FRONT_FEEDER},
  {"IP7500",  NO_FRONT_FEEDER},
  {"IP6600D", NO_FRONT_FEEDER},
  {"MP500",   NO_FRONT_FEEDER},
  {"IP3300",  FRONT_FEEDER},
  {"IP4300",  NO_FRONT_FEEDER},
  {"MP510",   FRONT_FEEDER},
  {"MP600",   NO_FRONT_FEEDER},
  {"MP160",   NO_FRONT_FEEDER},
  {"IP2500",  NO_FRONT_FEEDER},
  {"IP1800",  NO_FRONT_FEEDER},
  {"IP90",    NO_FRONT_FEEDER},
  {ENDTAG, 0}
};
...
int chk_operatorcall_level1(ST_CUPSSTS *p_tbl, ST_PrinterStatus *p_sts)
{
  int ret;
  int msgid;
  int is_frontfeeder = NO_FRONT_FEEDER;
  int rsm_button = IS_RESUME;


  // Resume/OK/Start Button for Ver.2.70
  is_frontfeeder = setFrontFeederID(p_tbl);
  if( is_frontfeeder == ID_ERR_NO_ERROR )
    return ID_ERR_NO_ERROR;
			...
			*(p_sts->pMessageID) == ID_STS_MESSAGE_OC_PAPER_OUT)
  {
	...
	//Ver.2.70
  	if( (is_frontfeeder == FRONT_FEEDER) && (msgid == ID_STS_MESSAGE_OC_PAPER_OUT22) )
  ...
  {
	//Ver.2.70
  	if( is_frontfeeder == FRONT_FEEDER )
...
// frontfeeder for Ver.2.70
static int setFrontFeederID(ST_CUPSSTS *p_tbl)
{
  int i;
  int is_frontfeeder = ID_ERR_NO_ERROR;

	for(i=0; frontfeeder_tbl[i].pr_name!=ENDTAG; i++){
		if(	strncmp(p_tbl->pr_name, frontfeeder_tbl[i].pr_name, strlen(frontfeeder_tbl[i].pr_name)) == 0 ) {
			is_frontfeeder = frontfeeder_tbl[i].is_frontfeeder;
			break;
		}
    }
	is_frontfeeder = frontfeeder_tbl[i].is_frontfeeder;
	
	return(is_frontfeeder);
} 
