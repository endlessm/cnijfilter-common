...
)
{
	...
	while ( (cnclerr = bjf_image_init( &lpbjinfo->bjfimage )) > 0 ) {
		...
		for ( y=0; y<ImageHeight; y++ ){
			...
			/****** CNCL Source data ******/
			result = CNCL_SourceData( &CnclData );
			if ( result < 0 ){
				goto onErr;
			}
			...
			do {
				/****** CNCL Raster data ******/
				result = CNCL_RasterData( &CnclData );
				if ( result < 0 ) goto onErr;

				for ( i=0; i<CnclData.numBuf; i++ ){
					/* dump plural pages. */
					outCmdPlural( CnclData.rasterDataPtr[i].buf, CnclData.rasterDataPtr[i].size, lpbjinfo->prn, fd, root );
				}
			
				/****** CNCL Complete data ******/
				cnclerr = CNCL_CompleteData( &CnclData );
...
//next:
		...
		do {
			/****** Data out ******/
			result = CNCL_FlushData( &CnclData );
			if ( result < 0 ) goto onErr;
