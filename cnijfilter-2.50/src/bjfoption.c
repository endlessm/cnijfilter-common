	...
	char 				*modelname )
{
	...
	if( *modelname != '\0' ){
		strncpy(bkup_modelname, modelname, sizeof(bkup_modelname));
		bkup_modelname[sizeof(bkup_modelname) - 1] = '\0';
	}else if (setopt[OPTINDEX(OPTMODEL)] & OPTBIT(OPTMODEL)) {
		strncpy(bkup_modelname, opt->model, sizeof(bkup_modelname));
		bkup_modelname[sizeof(bkup_modelname) - 1] = '\0';
		strncpy(modelname, opt->model, 256);
		modelname[255] = '\0';
	}
	else {
	...
	/* Check "borderless"  support & "papergap" support */
	...
	if (setopt[OPTINDEX(OPTEXTENSION)] & OPTBIT(OPTEXTENSION)) {
		...
		/* check format type (CNCL format) */
		if( (ext = CNCL_GetTableFormatType( &uidb.nominfo, (void *)bjlibdir )) <0 ){
			fprintf(stderr, "Error: INTERNAL ERROR\n");
			/* inappropriate selection error. */
			goto onError;
		}
		
		if( ext == CND_FORMATTYPE_4 ){	/* DB supports extension_level info */
