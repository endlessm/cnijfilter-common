...
RegiDialogTable regi_dialog_table[]=
{
	...
	{IDM_MX_860,	create_regi_type_i1_dialog,  create_regi_type_i2_dialog, create_regi_type_i3_dialog, NULL }, // 2008/11/28
	...
	{IDM_MP_250,	create_regi_type_j1_dialog,   create_regi_type_j2_dialog, create_regi_type_j3_dialog, NULL },// 2009/04/20
	{IDM_MP_270,	create_regi_type_j1_dialog,   create_regi_type_j2_dialog, create_regi_type_j3_dialog, NULL },// 2009/04/20
	{IDM_MP_490,	create_regi_type_j1_dialog,   create_regi_type_j2_dialog, create_regi_type_j3_dialog, NULL },// 2009/04/20
...
static void CheckAlignmentSetting2(GtkWidget *widget)
{
	...
	if ( !strcmp( model_name, IDM_IP_4600 ) || !strcmp( model_name, IDM_MP_630 ) ||
		 !strcmp( model_name, IDM_MP_550 ) || !strcmp( model_name, IDM_MP_560 ) || // 2009/04/21
		...
		if ( manual_head_mode == 0 ){	// auto
			...
			CheckAlignmentSetting2TypeAManual( widget );
				...
				!strcmp( model_name, IDM_MX_860 ) ) { // 2008/12/19
		CheckAlignmentSetting2TypeAManual( widget );
	}
	...
	short kindcnt)                  
{
	...
	model_name = GetModelName();		
	if ( !strcmp( model_name, IDM_MP_250 ) || !strcmp( model_name, IDM_MP_270 ) ||
		 !strcmp( model_name, IDM_MP_490 )){
...
void UtilCleaning(LPUIDB uidb)
{
	...
	if(	!strcmp(model_name, IDM_IP_3300) || 			
			...
			!strcmp(model_name, IDM_MX_860) || // 2008/11/28
	...
	if( !strcmp(model_name, IDM_MP_160) ||
		...
		*p = 0x00;
		...
		UtilNozzleCheck(uidb);
			   ...
			   !strcmp(model_name, IDM_MX_320) || 
			   !strcmp(model_name, IDM_MX_330) ||
	  		   !strcmp(model_name, IDM_MP_250) || !strcmp(model_name, IDM_MP_270) || !strcmp(model_name, IDM_MP_490) || // 2008/04/21
			   !strcmp(model_name, IDM_MP_550) || !strcmp(model_name, IDM_MP_560) || 
			   !strcmp(model_name, IDM_IP_4700) || !strcmp(model_name, IDM_MP_640) ){
		int		n;
		CLEANCMD* clean_cmd;
...
void UtilRefreshing(LPUIDB uidb)
{
	...
	if ( !strcmp(model_name, IDM_MX_860 ) ||
		...
		UtilRefreshingTypeA( uidb );
		 	  ...
		 	  !strcmp(model_name, IDM_MP_250 ) || !strcmp(model_name, IDM_MP_270 ) ||  !strcmp(model_name, IDM_MP_490 ) ){ // 2009/04/21
...
void UtilNozzleCheck(LPUIDB uidb)
{
	...
	if( !strcmp(model_name, IDM_IP_2500) || !strcmp(model_name, IDM_IP_1800) ||
		...
		GdkPixmap *ok_map, *ng_map, *iconMask;
		int i,n;
		int __attribute__ ((unused)) fname_length;
		char fnamebuf[256];
		char bufModelName[64];
		char ok_buf[256], ng_buf[256];
		...
		*p = 0x00;
		...
		!strcmp(model_name, IDM_MP_550) || !strcmp(model_name, IDM_MP_560) || 
		!strcmp(model_name, IDM_IP_4700) || !strcmp(model_name, IDM_MP_640) ){	// 2008/11/26
...
static void UtilNozzleCheckWithoutCheckPattern(LPUIDB uidb)
{
	...
	else if ( !strcmp(model_name, IDM_MX_320) || !strcmp(model_name, IDM_MX_330) ||	// 2008/11/28
			  !strcmp(model_name, IDM_MP_250) || !strcmp(model_name, IDM_MP_270) || !strcmp(model_name, IDM_MP_490) ){	// 2009/04/21
...
void UtilHeadAdjust(LPUIDB uidb)
{
	...
	else if( !strcmp(model_name, IDM_MP_540) || !strcmp(model_name, IDM_MX_860) ){  // 2008/11/28
		RegiTypeIManual( uidb );
	}
...
void UtilRollerCleaning(LPUIDB uidb)
{
	...
	if( !strcmp(model_name, IDM_MP_160) ||
		...
		RollerCleaningTypeB();
			  ...
			  !strcmp(model_name, IDM_IP_4700 )  ){ // 2009/04/21
		RollerCleaningTypeA( 0 );
	}
	else if ( !strcmp(model_name, IDM_MP_540 ) || !strcmp(model_name, IDM_MP_630 ) || !strcmp(model_name, IDM_MX_860 ) ||
			  !strcmp(model_name, IDM_MP_550 ) || !strcmp(model_name, IDM_MP_560 ) || !strcmp(model_name, IDM_MP_640 )){ // 2009/04/21
...
void UtilPowerOff(LPUIDB uidb)
{
	...
	if(UtilMessageBox(LookupText(g_keytext_list, "utility_message_8"),
		...
		*p = 0x00;
		...
		PutDeviceData(buf, BJLLEN+BJLSTARTLEN+BJLPOWEROFFLEN+BJLENDLEN+1, "POWEROFF"); // 2009/04/24
	}
}
...
static void AlertPreventPaperAbrasion( void )
{
	...
	if ( !strcmp(model_name, IDM_IP_3600) ||  !strcmp(model_name, IDM_IP_4600) ||
		 ...
		 !strcmp(model_name, IDM_MP_250) ||  !strcmp(model_name, IDM_MP_270) || !strcmp(model_name, IDM_MP_490) ||
		 !strcmp(model_name, IDM_MP_550) ||  !strcmp(model_name, IDM_MP_560) || 
		 !strcmp(model_name, IDM_IP_4700) ||  !strcmp(model_name, IDM_MP_640)   ){ // 2009/04/21
