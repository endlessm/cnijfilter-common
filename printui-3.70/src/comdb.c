...
/* Ver.3.60 */
static gboolean IsSupportValue(short id, short value)
{
	LPCNCLDB lpdb = g_uidb.lpdbTop;
	short i;
...
short NameToValue(short id, char* name)
{
	...
	for( i = 0 ; i < count ; i++ )
	{
		/* Ver.3.60 : permit that there is the same name in the setting item.  */
		if ( (id == CNCL_MEDIASUPPLY) && (value_array[i] == CND_SUPPLY_FRONT_FOR_PLAIN) ){
			;
		} 
		else {
			if ( IsSupportValue( id, value_array[i] ) == FALSE ) continue;
		}
...
/* Ver.3.50 */
short GetCurrentSizeGroup( short size_id )
{
	short i;
	short result = -1;

	i = 0;
	while ( g_size_group_info[i].size_id != -1 ){
		if ( g_size_group_info[i].size_id == size_id ){
			result = g_size_group_info[i].group;
			break;
		}
		i++;
	}
...
/*
	Function  : Get strings related version.
	Return    : gchar *
              : strings
	Arg       : short id
		      : 0 -> product name
		      ...
		      : 3 -> copyright2
*/
static gchar *GetVersionString( short id )
{
	return LookupText(g_keytext_list, g_version_param[id]);
}
...
*/
gchar *GetProductName( void ) { return GetVersionString( 0 ); }

/*
	Function  : Get version
*/
gchar *GetVersion( void ) { return GetVersionString( 1 ); }

/*
	Function  : Get copyright1.
*/
gchar *GetCopyright1( void ) { return GetVersionString( 2 ); }

/*
	Function  : Get copyright2.
*/
gchar *GetCopyright2( void ) { return GetVersionString( 3 ); }
