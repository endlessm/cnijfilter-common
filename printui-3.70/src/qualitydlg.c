...
UIQualityDialog* CreateQualityDialog(UIDialog* parent)
{
	...
	/* Hide Halftone Settings after CND_FormatType12 (Ver.3.60) */
	if ( type >= CND_FORMATTYPE_12 ){
		gtk_widget_hide( LookupWidget(window, "quality_label01") );
		gtk_widget_hide( LookupWidget(window, "custom_dialog_separator01") );
		gtk_widget_hide( LookupWidget(window, "custom_dialog_hbox02") );
	}	
...
void UpdateQualityDialogWidgets(GtkWidget* window, gchar* except_name)
{
...

	{
		...
		{	/* Ver.2.70: Write 5position value always */
			...
			snprintf(message, 16, "%d", 5 - index); /* Ver3.60 */
			gtk_label_set_text(
				GTK_LABEL(
