...
/* Ver.3.50 */
#define SEPARATOR_BUF_MAX 30
static int g_separator_pos[SEPARATOR_BUF_MAX];
...
static gboolean is_separator (GtkTreeModel *model, GtkTreeIter *iter, gpointer data)
{
	...
	if ( gtk_tree_model_get_iter_first( model, &lIter ) == FALSE ){
		goto EXIT;
	}
	...
	for( i=0; i<SEPARATOR_BUF_MAX; i++ ){
		if ( g_separator_pos[i] == -1 ) break;

		if ( gtk_tree_path_get_indices (path)[0] == g_separator_pos[i]) {
			result = TRUE;
		}
	}
	...
	/* Ver.3.00: Show supply position */
	{
		...
		if( GetCurrentnValue(CNCL_MEDIASUPPLY) == CND_SUPPLY_AUTO_SELECT )
		{
			if( ( media==CND_MEDIA_PLAIN ) && ( size==CND_SIZE_A4 || size==CND_SIZE_A5 || size==CND_SIZE_B5 || size==CND_SIZE_LETTER ) ){
				paperSource = GetMSIPaperSourceSettingsCurrentValue();
				if ( paperSource != -1 ){
					if (  paperSource == CND_SUPPLY_CASSETTE ){
						gtk_label_set_text(GTK_LABEL(supply_mes_label2), (gchar*)ValueToName( CNCL_MEDIASUPPLY , CND_SUPPLY_CASSETTE_02 ));
...
/* Ver.3.50 */
/* Make a list of available items of specified object , and Set the list to GtkComboBox with separator */
short SetItemsToSizeComboBox(GtkWidget* window)
{
	GtkWidget* combo = LookupWidget(window, "media_size_combo");
	short objectid = CNCL_PAPERSIZE;
	short currentid = GetCurrentnValue(CNCL_PAPERSIZE);

	LPCNCLDB lpdb = g_uidb.lpdbTop;
	char *str;
	short i,active_item,item_count;
	short currentGrp, sep_pos;
	...
	active_item = 0;
	sep_pos = 0;
	for( i=0; i<SEPARATOR_BUF_MAX; i++ )  g_separator_pos[i] = -1;
	currentGrp = 1;
	for( i = 0 ; i < g_uidb.dbsize ; i++, lpdb++ )
	{
		if( lpdb->nObjectID == objectid )
		{
			...
			{
				if( *str != '\0' && lpdb->disable == 0 ){
					if ( currentGrp != GetCurrentSizeGroup( lpdb->nValue ) ){
						gtk_combo_box_append_text( GTK_COMBO_BOX( combo ), "-" );
						currentGrp = GetCurrentSizeGroup( lpdb->nValue );
						g_separator_pos[sep_pos] = item_count;
						sep_pos++;
						item_count++;
					}
					/* append one item */
