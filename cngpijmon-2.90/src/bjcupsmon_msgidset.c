...
//for Ver.2.90
static int chk_inkout_DOCEMPTY_CTKIO(ST_CUPSSTS *p_tbl, ST_PrinterStatus *p_sts, int switch_IOIUK);
...
static int setSupportDeviceID(ST_CUPSSTS *p_tbl);
static int setPrheadLockLever(ST_CUPSSTS *p_tbl);
...
//Ver.2.70 beta02 2.00.01.14   Don't use "0-F"!  ex) 6B00 is NG.
  {'G', ID_STS_MESSAGE_SC_BATTERY_ERROR},
...
ST_MSGTBL operatelevel2[]={
  ...
  {'T', ID_STS_MESSAGE_OC_INKTANK_NOTPROPERLY},
...
ST_MSGTBL inklowpart1[]={
  ...
  {'B', ID_STS_MESSAGE_WAR_INKLOW_BLACK19},	// iP100
  {'G', ID_STS_MESSAGE_WAR_INKLOW_COLOR19},	// iP100
...
ST_MSGTBL inklowpart1_type_OTH[]={
  ...
  {'B', ID_STS_MESSAGE_WAR_INKLOW_BLACK35},	// iP100
  {'G', ID_STS_MESSAGE_WAR_INKLOW_COLOR36},	// iP100
  {'b', ID_STS_MESSAGE_WAR_INKLOW_BLACK15},	// iP90
  {'e', ID_STS_MESSAGE_WAR_INKLOW_COLOR16},	// iP90
  {ENDTAG, 0}
};
...
//Ver.2.90 for B-21 message switch
//for Printhead lock lever only
static const
ST_SUPPORT_PRHEAD_LOCKLEVER_TBL support_prhead_locklever[]={
  {"IP2200",  DONOT_CARE},
  {"IP4200",  DONOT_CARE},
  {"IP7500",  DONOT_CARE},
  {"IP6600D", DONOT_CARE},
...
//Ver.2.90 for B-20 message switch
//for bluetooth only
static const
ST_SUPPORT_DEVICE_TBL support_device[]={
  {"IP2200",  DONOT_CARE},
  {"IP4200",  DONOT_CARE},
  {"IP7500",  DONOT_CARE},
  {"IP6600D", DONOT_CARE},
...
ST_MSGTBL cartridge_type_ZZZ[]={ //for Ver.2.90
  {'U', ID_STS_MESSAGE_OC_BLACK},
  {'V', ID_STS_MESSAGE_OC_COLOR},
  {'L', ID_STS_MESSAGE_OC_BLACK},
  {'M', ID_STS_MESSAGE_OC_COLOR},
  {'N', ID_STS_MESSAGE_OC_BLACK},
  {'O', ID_STS_MESSAGE_OC_COLOR},
  {ENDTAG, 0}
};
...
ST_REGION_TBL cartridge_region_tbl[]={
  ...
  {'E', cartridge_type_EU},
  {'Z', cartridge_type_ZZZ}, //for Ver.2.90
  {ENDTAG, 0}
};
...
ST_MSGTBL color_type_JP[]={
  ...
  {'B', ID_STS_MESSAGE_OC_BLACK19}, //Ver.2.90 for iP100
  {'G', ID_STS_MESSAGE_OC_COLOR19}, //Ver.2.90 for iP100
  {ENDTAG, 0}
};
...
ST_MSGTBL color_type_OTH[]={
  ...
  {'B', ID_STS_MESSAGE_OC_BLACK35}, //Ver.2.90 for iP100
  {'G', ID_STS_MESSAGE_OC_COLOR36}, //Ver.2.90 for iP100
  {ENDTAG, 0}
};
...
int chk_operatorcall_level1(ST_CUPSSTS *p_tbl, ST_PrinterStatus *p_sts)
{
  ...
  int rsm_button = ID_ERR_NO_ERROR; //Ver.2.90 change
  //int rsm_button = IS_RESUME;
  int is_support_device = ID_ERR_NO_ERROR;


  // Support Device for Ver.2.90
  is_support_device = setSupportDeviceID(p_tbl);
  if( is_support_device == ID_ERR_NO_ERROR )
    return ID_ERR_NO_ERROR;
  ...
  //Ver.2.90 for B-20 message switch
  if( *(p_sts->pMessageID) == ID_STS_MESSAGE_OC_USBDEVICE && is_support_device == BLUE_TOOTH )
      *(p_sts->pMessageID)=ID_STS_MESSAGE_OC_USBDEVICE_BLUETOOTH;
...
int chk_operatorcall_level2(ST_CUPSSTS *p_tbl, ST_PrinterStatus *p_sts)
{
  ...
  int rsm_button = IS_RESUME;
  int color;
  int is_prhead_locklever = ID_ERR_NO_ERROR;
  ...
  // Printhead Lock Lever for Ver.2.90
  is_prhead_locklever = setPrheadLockLever(p_tbl);
  if( is_prhead_locklever == ID_ERR_NO_ERROR )
    return ID_ERR_NO_ERROR;
  ...
  //Ver.2.90 for B-21 message switch
  if( *(p_sts->pMessageID) == ID_STS_MESSAGE_OC_INKTANK_NOTPROPERLY && is_prhead_locklever == LOCK_LEVER )
  {
      *(p_sts->pMessageID)=ID_STS_MESSAGE_OC_INKTANK_NOTPROPERLY_LOCKLEVER_01;
      ret=storeMessageID(p_sts, ID_STS_MESSAGE_OC_INKTANK_NOTPROPERLY_LOCKLEVER_02);
      ret=storeMessageID(p_sts, ID_STS_MESSAGE_OC_INKTANK_NOTPROPERLY_LOCKLEVER_03);
      ret = UNIQUESTS;
   }
...
{
  int 		ret;
  int 		msgid,fourth_msgid=ID_STS_MESSAGE_NON;
  int 		third_msgid=ID_STS_MESSAGE_NON;
  int 		color;
  int  		i, j, k, m, x;
  int		EMPTYIO_flag = 0;
  int		tmpMessageID;
	  ...
	  for(i=0; msg_tbl_inkout[msgid].id[i]!=ID_STS_MESSAGE_NON; i++)
	  {
	    if(msg_tbl_inkout[msgid].id[i] == ID_STS_MESSAGE_COLOR)
	    {
	      ...
	      for(j=0; j<MAXINKNUM; j++)
	      {
	        ...
	        //check EMPTY("Y" or "r" r:iP100) && Inkout
	        if( msg_tbl_inkout[msgid].status == 'Y' && p_tbl->ink[j].alert == 'r' ){
	        	//printf("EMPTY & UNKNOWN\n");
	        	EMPTYIO_flag = 2;
	        }
	        if(  (msg_tbl_inkout[msgid].status == 'r' || msg_tbl_inkout[msgid].status == 'Y') && p_tbl->ink[j].alert == 'I' ){
	        	//printf("UNKNOWN & INKOUT / EMPTY & INKOUT\n");
	        	EMPTYIO_flag = 1;
	        }
  ...
  //for Dispo Ink
   ...
   //for set Ink
	...
	//for Ver2.90(iP100)
	if( (EMPTYIO_flag == 1) || (EMPTYIO_flag == 2) ){
		ret = chk_inkout_DOCEMPTY_CTKIO(p_tbl, p_sts, EMPTYIO_flag);
...
//for Ver.2.70 and 2.90
//DOC 1688(EMPTY) or 1689(EMPTY) && CTK:IO or CTK:IUK
//Operator Call 'C' or 'c' only
int chk_inkout_DOCEMPTY_CTKIO(ST_CUPSSTS *p_tbl, ST_PrinterStatus *p_sts, int switch_IO_IUK)
//int chk_inkout_DOCEMPTY_CTKIO(ST_CUPSSTS *p_tbl, ST_PrinterStatus *p_sts)
{
  ...
  int rsm_button = IS_RESUME;
    ...
    for(k=0; (k<sizeof(p_tbl->operator_call)) && (*(p_tbl->operator_call+k)!=SPACE); k++){
		//for iP100
		if( (p_tbl->operator_call[k] == 'C') || ((p_tbl->operator_call[k] == 'c')) ){
   ...
   if( rsm_button == ID_ERR_NO_ERROR )
	...
	//DOC 1689(EMPTY) && CTK:IUK
	} else if( switch_IO_IUK == 2 ){
		if( IO_Flag == 1 ){
	   	if( rsm_button == IS_OK )
	        *(p_sts->pMessageID) = ID_STS_MESSAGE_OC_INKEMPTY0602_TANK_3rd;
		else
	  ...
	  // The first message is inserted.
		ret = storeMessageID (p_sts, ID_STS_MESSAGE_OC_INKEMPTY0602_TANK_1st);
		if(ret != ID_ERR_NO_ERROR)
			return ret;
...
static int setSupportDeviceID(ST_CUPSSTS *p_tbl)
{
  ...
  int is_support_device = ID_ERR_NO_ERROR;
	...
	for(i=0; support_device[i].pr_name!=ENDTAG; i++){
		if(	strncmp(p_tbl->pr_name, support_device[i].pr_name, strlen(support_device[i].pr_name)) == 0 ) {
			is_support_device = support_device[i].is_support_device;
			break;
		}
    }
	is_support_device = support_device[i].is_support_device;
	
	return(is_support_device);
} 
...
static int setPrheadLockLever(ST_CUPSSTS *p_tbl)
{
  ...
  int is_prhead_locklever = ID_ERR_NO_ERROR;
	...
	for(i=0; support_prhead_locklever[i].pr_name!=ENDTAG; i++){
		if(	strncmp(p_tbl->pr_name, support_prhead_locklever[i].pr_name, strlen(support_prhead_locklever[i].pr_name)) == 0 ) {
			is_prhead_locklever = support_prhead_locklever[i].is_prhead_locklever;
			break;
		}
    }
	is_prhead_locklever = support_prhead_locklever[i].is_prhead_locklever;
	
	return(is_prhead_locklever);
} 
