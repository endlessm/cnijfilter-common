...
static	char	*power_on_mode;
static	char	*power_off_mode;	
static	int poweron_index=0;		// Added '01.12.26
static	int poweroff_index=0;		// Added '01.12.26
static	int drylevel_value;		// Added '01.11.05
static 	int quiet_mode;			// Added '01.11.05
static	int warning_mode;		// Added '01.11.16
static 	int black_reset;		// Added '01.11.16
static	int color_reset;		// Added '01.11.16
...
#else
	regi_dialog = create_regi_dialog();		
#endif
...
static void drylevel_set_default(GtkWidget *widget)
{
	...
	}else if( !strcmp(model_name, "BJS500") || !strcmp(model_name, "PIXUS550I")
...
static void SetDryLevel(void)
{
	...
	if(!strcmp(model_name, "BJF900") || !strcmp(model_name, "BJF9000") 
		...
		gtk_window_set_transient_for(GTK_WINDOW(drylevel_dialog),
								 GTK_WINDOW(UI_DIALOG(g_main_window)->window));
		button = LookupWidget(drylevel_dialog, "drylevel3_radiobutton1");
		...
		button = LookupWidget(drylevel_dialog, "drylevel5_radiobutton3");
	...
	if(UtilMessageBox(LookupText(g_keytext_list, "utility_message_9"),
					  ...
					  MB_CANCEL)==ID_CANCEL){
		drylevel_set_default(drylevel_dialog);
		return ;
	}
	...
	memcpy(p, BJLEND, BJLENDLEN); p+=BJLENDLEN;
	*p = 0x00;
	total_bytes = BJLLEN + BJLSTARTLEN
						+ BJLCTRLMODELEN + cmd_bytes + BJLENDLEN +1;
...
void UtilCleaning(LPUIDB uidb)
{
	...
	if (UtilMessageBox(LookupText(g_keytext_list, "utility_message_11"),
	...
	if (!strcmp(model_name, "BJF850") | !strcmp(model_name, "BJF860") |
		...
		*p = 0x00;
		...
		memcpy(p, bjl_cmds, BJLLEN); p+=BJLLEN;						
		memcpy(p, BJLSTART, BJLSTARTLEN); p+=BJLSTARTLEN;           
		memcpy(p, S600CleanCommand[head_type].cmd,
			S600CleanCommand[head_type].len);	
		p+=S600CleanCommand[head_type].len;
		memcpy(p, BJLEND, BJLENDLEN); p+=BJLENDLEN;                 
		*p = 0x00;
		PutPrintData(buf,
			n+BJLLEN+BJLSTARTLEN+S600CleanCommand[head_type].len+BJLENDLEN+1);
	}
...
void UtilRefreshing(LPUIDB uidb)
{
	...
	if ( !strcmp(model_name, "PIXUS550I") || !strcmp(model_name, "PIXUS850I")){
		...
		switch( refresh_head_type ){
			
		case 0:
			msg = LookupText(g_keytext_list, "utility_message_12");
			break;
		case 1:
			msg = LookupText(g_keytext_list, "utility_message_23");
			break;
		case 2:
			msg = LookupText(g_keytext_list, "utility_message_24");
			break;
		}
	}
	...
	if (!strcmp(model_name, "BJF850") | !strcmp(model_name, "BJF860") |
		...
		*p = 0x00;
		...
		memcpy(p, bjl_cmds, BJLLEN); p+=BJLLEN;						
		memcpy(p, BJLSTART, BJLSTARTLEN); p+=BJLSTARTLEN;           
		memcpy(p, RefreshingCommand[refresh_head_type].cmd,
			   RefreshingCommand[refresh_head_type].len);	
		p+=RefreshingCommand[refresh_head_type].len;
		memcpy(p, BJLEND, BJLENDLEN); p+=BJLENDLEN;                 
		*p = 0x00;
		PutPrintData(buf, n+BJLLEN+BJLSTARTLEN+
					 RefreshingCommand[refresh_head_type].len+BJLENDLEN+1);
...
void UtilNozzleCheck(LPUIDB uidb)
{
	...
	if (UtilMessageBox(LookupText(g_keytext_list, "utility_message_7"),
		...
		if (!strcmp(model_name, "BJS600") | !strcmp(model_name, "BJS630") |
			!strcmp(model_name, "BJS6300") | !strcmp(model_name, "BJF900") | 
			!strcmp(model_name, "BJF9000") | !strcmp(model_name, "BJS300") |
			...
			!strcmp(model_name, "PIXUS850I") | !strcmp(model_name,"PIXUS950I"))
		  {
			...
			memcpy(p, bjl_cmds, BJLLEN);p+=BJLLEN;
			memcpy(p, BJLSTART, BJLSTARTLEN);p+=BJLSTARTLEN;
			memcpy(p, BJLNOZZLECHECK, BJLNOZZLECHECKLEN);p+=BJLNOZZLECHECKLEN;
			memcpy(p, BJLEND, BJLENDLEN);p+=BJLENDLEN;
			*p = 0x00;
			PutPrintData(buf,
...
void UtilRollerCleaning(LPUIDB uidb)
{
	...
	if (!strcmp(model_name,"BJS600") || !strcmp(model_name,"BJS630") ||
		!strcmp(model_name,"BJS6300") || !strcmp(model_name,"BJF900") ||
		!strcmp(model_name,"BJF900") || !strcmp(model_name,"BJS300") ||
...
void UtilAutoPower(LPUIDB uidb)
{
	...
	param = GetParamValue(AutoPowerOnKey, AutoPowerOnValue, 2, power_on_mode);	
	memcpy (p, bjl_cmds, BJLLEN); p+=BJLLEN;				
	memcpy (p, BJLSTART, BJLSTARTLEN); p+=BJLSTARTLEN;		
	memcpy(p, BJLCTRLMODE, BJLCTRLMODELEN); p+=BJLCTRLMODELEN;	
	...
	param = GetParamValue(
				AutoPowerOffKey, AutoPowerOffValue, 6, power_off_mode);
...
void UtilInkWarning(LPUIDB uidb)
{
	...
	gtk_main();									
 	...
 	if(UtilMessageBox(LookupText(g_keytext_list, "utility_message_9"),
		g_window_title, MB_ICON_EXCLAMATION | MB_OK | MB_CANCEL)==ID_CANCEL){
		ink_warning_set_default(warning_dialog);
 		return ;
	}
...
void UtilQuietMode(LPUIDB uidb)
{
	...
	if (strcmp(model_name,"BJF900") && strcmp(model_name,"BJF9000") && 
		strcmp( model_name, "PIXUS550I") && strcmp(model_name, "PIXUS850I") &&
		strcmp( model_name, "PIXUS950I") )
		return;
	...
	gtk_main();									
 	...
 	if(UtilMessageBox(LookupText(g_keytext_list, "utility_message_9"),
		g_window_title, MB_ICON_EXCLAMATION | MB_OK | MB_CANCEL)==ID_CANCEL){
		quiet_set_default(quiet_dialog);
 		return ;
	}
