...
const char usage_str_dpw[]		= "                 [ --detectpaperwidth                     ]\n";
...
)
{
	...
	struct poptOption optionsTable[] = {
		...
		{ OPTSTRDETECTPW,		0,		POPT_ARG_NONE,		&opt.detectpaperwidth,	OPTDETECTPW},
		{ NULL,					0,		0,					NULL,					0 			}
	};
	...
	char 				*modelname )
{
	...
	*/
	...
	if (FindValue(uidb.lpdbTop, uidb.dbsize, CNCL_PWDETECTION_TYPE, CND_PWDETECTION_TYPE_01) >= 0) {
		dpw_support = 1;
	}
	...
	/* Ver.4.00 */
	lpbjf_optinfo->detect_pw = -1;
	if (formattype >= CND_FORMATTYPE_14){
		if (dpw_support == 1) {
			if (setopt[OPTINDEX(OPTDETECTPW)] & OPTBIT(OPTDETECTPW)) {
				lpbjf_optinfo->detect_pw = 1;
			}
			else
			{
				lpbjf_optinfo->detect_pw = 0;
			}
		}
	}
...
short CheckSettings( LPBJFLTDEVICE bjdevice , char *confname )
{
	...
	if( bjdevice->bjfltModelID >= CND_MG6300 ){
		if ( bjdevice->bjfltMediaType == CND_MEDIA_OTHER_FINE_ART_PAPER ){
			if (( bjdevice->bjfltPaperSize != CND_SIZE_A4_ART_35_2 ) && ( bjdevice->bjfltPaperSize != CND_SIZE_LETTER_ART_35_2 ) && ( bjdevice->bjfltPaperSize != CND_SIZE_A3_ART_35_2 ) && ( bjdevice->bjfltPaperSize != CND_SIZE_A3_PLUS_ART_35_2 )){
