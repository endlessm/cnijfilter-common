...
)
{
	...
	char				fdpw_command[256];
	short				fdpw_command_size = sizeof(fdpw_command);
	...
	/****** CNCL SetForceDetectPW ******/
	if (lpbjinfo->bjfoption.detect_pw >= 0)
	{
		if (lpbjinfo->bjfoption.detect_pw != 0)
		{
			if ( (cnclerr = CNCL_SetForceDetectWidth(fdpw_command, &fdpw_command_size, 1)) != CNCL_OK ) goto finish1;
		} 
		else
		{
			if ( (cnclerr = CNCL_SetForceDetectWidth(fdpw_command, &fdpw_command_size, 0)) != CNCL_OK ) goto finish1;
		}
		outCmd( (unsigned char *)fdpw_command, fdpw_command_size, lpbjinfo->prn );
	}
