...
short ExecMaintenance( short funcType )
{
	...
	while( lpComp != NULL ) {
		if ( lpComp->type == funcType ){
			...
			if ( CopyFlowName( flowidNameTable, lpComp->flow, &flowNum ) != 0 ) goto Err;
			break;
		}
		lpComp = lpComp->next;
	}
	...
	while ( strcmp( flowidNameTable[current], "END" ) ){
		...
		while ( status != U_STATUS_END ) {
			...
			if ( GetFuncNameFromFlowList( list, flowidNameTable[current], status, &dialogName ) != 0 ) goto Err;
#ifdef _PRINTUI_DEBUG_
			...
			if ( GetCommandNameFromFLowList( list, flowidNameTable[current], status, action,  &cmdName ) != 0 )  goto Err;
#ifdef _PRINTUI_DEBUG_
			if ( cmdName != NULL ){
				printf( "cmdName : %s\n", cmdName );
			}
			...
			if ( GetNextIdFromFlowList( list, flowidNameTable[current], status, action, &next ) != 0 ) goto Err;
#ifdef _PRINTUI_DEBUG_
			printf( "next : %d\n", next );
			printf( "------------------------>[end]\n" );
#endif
