...
static void InitCreate_SetConfigType001_QuietMode( void )
{
	gQuietMode = 0;
	gCurrentQuietMode = 0;
}
...
static void InitParamSetConfigType001( LPSETCONFIGCOMPTYPE001RES lpRes, void *lpGWork )
{
	...
	if ( (lpWork->uiType == UI_CUSTOMSETTINGS_TYPE001) || (lpWork->uiType == UI_CUSTOMSETTINGS_TYPE002) ){
		if ( gIsFirstExecCustom ) {
			InitCreate_SetconfigType001_CustomSettings( lpWork->uiType );
			gIsFirstExecCustom = 0;
		}
	}
	else if ( (lpWork->uiType == UI_AUTOPOWER_TYPE001) || (lpWork->uiType == UI_AUTOPOWER_TYPE002) ){
...
static void SetDefaultSetConfigType001_CustomSettings( int uiType, GtkWidget *dialog, int isAbrasion, int isManualHead, int drylevelValue, int isPreventPaperJam  )
{
	GtkWidget *button;
...
static void SetDefaultSetConfigType001_AutoPowerType( int uiType, GtkWidget *dialog, short onIndex, short offIndex  )
{
	...
	if ( uiType == UI_AUTOPOWER_TYPE001 ){
		...
		SetComboBoxItems(powerOnCombo, setconfig_type001_02_onlist, onSize, onIndex);		
		SetComboBoxItems(powerOffCombo, setconfig_type001_03list, offSize , offIndex);
	}
...
static void SetDefaultSetConfigType001( LPSETCONFIGCOMPTYPE001RES lpRes, void *lpGWork )
{
	...
	if ( (lpWork->uiType == UI_CUSTOMSETTINGS_TYPE001) || (lpWork->uiType == UI_CUSTOMSETTINGS_TYPE002) ){
		SetDefaultSetConfigType001_CustomSettings( lpWork->uiType, lpWork->dialog, gAbrasionPrevention, gManualHeadMode,  gDryLevelValue, gPreventPaperJam );
		//SetDefaultSetConfigType001_CustomSettings( lpWork->uiType, lpWork->dialog, 0, 0, CUSTOM_SETTINGS_TYPE001_RADIOBUTTON_DEFAULT, 0 );
	}
	else if ( (lpWork->uiType == UI_AUTOPOWER_TYPE001) || (lpWork->uiType == UI_AUTOPOWER_TYPE002) ){
		SetDefaultSetConfigType001_AutoPowerType( lpWork->uiType, lpWork->dialog, gAutoPowerOnIndex, gAutoPowerOffIndex );
	}
	else if ( lpWork->uiType == UI_PAPERSOURCESETTINGS_TYPE001 ){
		SetDefaultSetConfigType001_PaperSourceSettings( lpWork->dialog, gPaperSourceSettingsValue );
		//SetDefaultSetConfigType001_PaperSourceSettings( lpWork->dialog, PAPERSOURCE_SETTINGS_TYPE001_RADIOBUTTON_DEFAULT );
	}
	else if ( lpWork->uiType ==  UI_QUIET_TYPE001 ){
                                        ...
                                        gpointer         user_data)
{
	...
	if ( (lpWk->uiType == UI_CUSTOMSETTINGS_TYPE001) || (lpWk->uiType == UI_CUSTOMSETTINGS_TYPE002) ){
		GetParmSetConfigType001_CustomSettings( lpWk->uiType, button );
	}
	else if ( (lpWk->uiType == UI_AUTOPOWER_TYPE001) || (lpWk->uiType == UI_AUTOPOWER_TYPE002) ) {
		GetParmSetConfigType001_AutoPowerType( lpWk->uiType, button );
	}	
                                        ...
                                        gpointer         user_data)
{
	...
	if ( (gUiType == UI_CUSTOMSETTINGS_TYPE001) || (gUiType == UI_CUSTOMSETTINGS_TYPE002) ){
		SetDefaultSetConfigType001_CustomSettings( gUiType, (GtkWidget *)button, 0, 0, CUSTOM_SETTINGS_TYPE001_RADIOBUTTON_DEFAULT, 0 );
	}	
	else if ( gUiType == UI_PAPERSOURCESETTINGS_TYPE001 ) {
		SetDefaultSetConfigType001_PaperSourceSettings( (GtkWidget *)button, PAPERSOURCE_SETTINGS_TYPE001_RADIOBUTTON_DEFAULT );
	}	
	else if ( gUiType ==  UI_QUIET_TYPE001 ){
		SetDefaultSetConfigType001_QuietMode( (GtkWidget *)button, 0 );
	}
Err:
	return;
}
...
static short SetCurrentStatusSetConfigType002( LPSETCONFIGCOMPTYPE002RES lpRes, void *lpGWork )
{
	...
	if( lpWork->status == US_OK ){
		...
		else if ( (gUiType == UI_AUTOPOWER_TYPE001) || (gUiType == UI_AUTOPOWER_TYPE002) ) {
			/* send bjl command */
			/* renew global data */
